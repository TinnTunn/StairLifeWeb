 ...
 * Depends on: UsersAPI, showToast, animateProgressBar, updateStudentVerificationUI,
 *             loadProjectsFromAPI, renderApplications, renderActiveProjects, refreshNotifBadge.
...
async function onEnterStudent() {
  try {
    ...
    if (greetEl) greetEl.textContent = `Halo, ${user.full_name || 'Pengguna'}! 👋`;

    const subtitleEl = document.querySelector('.dash-subtitle');
    if (subtitleEl && user.university) {
      subtitleEl.textContent = `${user.university} · ${user.major || ''} · Semester ${user.semester || ''}`;
    }

    const fields = [
      user.full_name, user.email, user.phone, user.university,
      user.major, user.semester, user.bio, user.portfolio_url,
      Array.isArray(user.skills) && user.skills.length > 0,
      user.avatar_url, user.is_verified, !!user.bank_account,
    ];
...
async function loadStudentActivity() {
  ...
  try {
    // Gabungkan dari: applications terbaru + contracts + notifications
    ...
    // Sort by time desc, ambil 5 terbaru
...
async function switchStudentTab(i) {
  document.querySelectorAll('#screen-student .sidebar-item').forEach(el => el.classList.toggle('active', parseInt(el.dataset.tab) === i));
  document.querySelectorAll('#student-bnav .bnav-item').forEach(el => el.classList.toggle('active', parseInt(el.dataset.tab) === i));
  document.querySelectorAll('[id^="st-tab-"]').forEach(el => el.classList.toggle('active', el.id === `st-tab-${i}`));
...
async function loadStudentProfile() {
  try {
    ...
    // L4 FIX: jangan pakai is_verified saja (boolean) — fetch status verifikasi asli
    // agar status 'pending' dan 'rejected' juga ter-reflect dengan benar
    updateStudentVerificationUI();

    const skillsEl = document.querySelector('#st-tab-4 .skill-tag:first-of-type')?.parentElement;

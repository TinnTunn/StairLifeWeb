  ...
  try {
    ...
    listEl.innerHTML = apps.map(a => {
      ...
      if (a.status === 'pending') {
        ...
        actionButtons = `<span class="badge badge-teal">✅ Diterima</span><button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();openDirectChatWith('${name}','mahasiswa','${projectId}')">💬 Chat</button>`;

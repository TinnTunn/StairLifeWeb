...
async function onEnterBusiness() {
  try {
    ...
    const user = res.data;
    const greetEl = document.querySelector('#screen-business .dash-greeting');
    if (greetEl) greetEl.textContent = `Selamat datang, ${user.full_name}! 🏢`;
  } catch (e) {}
...
async function bizOpenInquiryChat(inquiryId, senderName, projectTitle, senderId) {
  ...
  try {
    const res      = await MessagesAPI.replyInquiry(inquiryId, '');
    // Ini hanya untuk membuka room — reply kosong tidak akan tersimpan
    // TODO: load history inquiry dari backend
  } catch (_) {}
  ...
  // Setup send untuk balasan inquiry

...
function getChatSubtitle(chat) {
  if (chat.type === 'mediation') return `${chat.biz || 'Bisnis'} · ${chat.student || 'Mahasiswa'} · Admin`;
...
async function openSupportChat() {
  ...
  try {
    ...
    if (!chat) {
      chat = {
        id, type: 'support', members: [role, 'admin'],
        biz: `Support · ${role === 'student' ? 'Mahasiswa' : 'Bisnis'}`,
        last: room.last_message || 'Halo! Ada yang bisa kami bantu?',
        time: room.updated_at ? new Date(room.updated_at) : new Date(),
        unreadBy: { student: 0, biz: 0, admin: 0 }, online: true,
        backendRoomId: room.id || null, isBackend: true,
      };
      CHATS.unshift(chat);
    }
    if (Array.isArray(room.messages)) {
      CHAT_MESSAGES[id] = room.messages.map(m => ({
        text: m.content, senderRole: m.sender_role || (m.sender_id === room.admin_id ? 'admin' : role),
        senderName: m.sender_name || (m.sender_role === 'admin' ? 'Admin Support' : getChatDisplayName()),
        time: m.created_at ? new Date(m.created_at) : new Date(),
      }));
    } else if (!CHAT_MESSAGES[id]) {
      CHAT_MESSAGES[id] = [
        { text: 'Halo! Saya Admin Support StairsLife. Silakan jelaskan kendala kamu ya 😊', senderRole: 'admin', senderName: 'Admin Support', time: new Date() },
      ];
    }
    openChatRoom(id);
  } catch (error) {
    console.warn('[support chat] backend fallback:', error.message);
...
function renderChatContextBar() {
  ...
  if (chat.type === 'mediation' && role === 'admin') {
    ...
    el.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap">
        <div style="font-size:13px;color:var(--text-secondary)"><b>Template keputusan</b> (demo): kirim keputusan + update status dispute.</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button class="btn btn-ghost btn-sm" onclick="adminDecision('${chat.disputeId}','refund')">Refund</button>
          <button class="btn btn-ghost btn-sm" onclick="adminDecision('${chat.disputeId}','partial')">Partial</button>
          <button class="btn btn-primary btn-sm" onclick="adminDecision('${chat.disputeId}','continue')">Continue</button>
        </div>
      </div>`;
    return;
  }
...
/**
 * stopChatPolling — dipertahankan sebagai no-op untuk backward compat
 * (masih dipanggil dari beberapa tempat di codebase)
 */
function stopChatPolling() {
  if (_chatPollingInterval) { clearInterval(_chatPollingInterval); _chatPollingInterval = null; }
  // Juga leave room WebSocket jika ada
...
/**
 * startChatPolling — sekarang menjadi wrapper ke WebSocket.
 * Nama dipertahankan supaya pemanggil lama tidak perlu diubah.
 */
function startChatPolling(contractId) {
  stopChatPolling();
  // Pakai WebSocket, bukan polling
...
async function sendChatMessage() {
  ...
  try {
    ...
    showToast('Pesan tersimpan lokal. Akan terkirim saat koneksi pulih.', 'info');
  }
}

 ...
 * onEnterVerification, renderVerifStep, submitVerification, updateStudentVerificationUI.
...
async function onEnterVerification() {
  // M4 FIX: selalu ambil status terbaru dari backend sebelum render
...
function buildVerifUploadMain() {
  ...
  <input type="file" id="verif-main-input" accept="image/jpeg,image/png,image/webp,application/pdf"
...
function buildVerifSelfie() {
  ...
  <input type="file" id="verif-selfie-input" accept="image/jpeg,image/png,image/webp"
...
function buildVerifReview() {
  const isKtm = VERIF.docType === 'ktm';
  return `
  <div style="background:var(--teal-light);border:1px solid rgba(20,184,166,0.3);border-radius:var(--radius-md);padding:14px;display:flex;align-items:center;gap:10px;margin-bottom:20px">
...
function verifBack()   {
  if (VERIF.step === 0) goBack();
  else if (VERIF.step === 4 && VERIF.docType === 'email') { VERIF.step = 2; renderVerifStep(); }
  else { VERIF.step--; renderVerifStep(); }
}
function verifNext()   { if (VERIF.step === 2 && VERIF.docType === 'email') { VERIF.step = 4; } else VERIF.step++; renderVerifStep(); }
...
async function pickMainDoc(event) {
  ...
  try {
    ...
    VERIF.mainDocName  = res?.data?.url || res?.url || file.name;
    VERIF._mainDocUrl  = res?.data?.url || res?.url || null;
    renderVerifStep();
    showToast('Dokumen berhasil diupload ✅', 'success');
  } catch (err) {
    // Fallback: simpan nama file lokal, URL akan digenerate saat submit
    VERIF.hasMainDoc  = true;
    VERIF.mainDocName = file.name;
    VERIF._mainDocFile = file;
    renderVerifStep();
    showToast('File dipilih ✅', 'success');
  }
...
async function pickSelfieDoc(event) {
  ...
  try {
    ...
    VERIF.selfieName  = res?.data?.url || res?.url || file.name;
    ...
    VERIF._selfieFile  = file;
    renderVerifStep();
    showToast('Selfie dipilih ✅', 'success');
  }
  if (event.target) event.target.value = '';
}
...
async function submitVerification() {
  if (!VERIF.hasMainDoc || !VERIF.mainDocName || VERIF.mainDocName === 'ktm_placeholder.jpg') {
  ...
  try {
    // Jika file belum terupload (fallback), upload sekarang
    let ktmUrl = VERIF._mainDocUrl || VERIF.mainDocName;
    if (VERIF._mainDocFile && !VERIF._mainDocUrl) {
      const up = await UploadAPI.uploadFile(VERIF._mainDocFile, 'ktm');
      ktmUrl = up?.data?.url || up?.url || VERIF.mainDocName;
    }
...
async function updateStudentVerificationUI() {
  try {
    ...
    console.error('Gagal cek verifikasi:', error);
  }

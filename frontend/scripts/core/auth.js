 ...
 * Login, register (multi-step), OTP, KTM upload, logout.
...
async function handleLogin() {
  ...
  try {
    ...
    // Sambung WebSocket setelah login berhasil
    if (typeof SocketManager !== 'undefined') SocketManager.connect();
    ...
    // Shake animation pada form untuk feedback visual
...
async function handleForgotPassword() {
  ...
  try {
    ...
    showToast(
      'Jika email kamu terdaftar, link reset password sudah dikirim. Cek inbox/spam ya.',
      'success',
    );
    setTimeout(() => goTo('screen-login'), 1500);
  }
...
function totalRegSteps() { return state.regRole === 'student' ? 5 : 4; }

function renderStepIndicator() {
  const total  = totalRegSteps();
  const labels = state.regRole === 'student'
    ? ['Peran', 'Data', 'OTP', 'KTM', 'Selesai']
    : ['Peran', 'Data', 'OTP', 'Selesai'];
  let html = '';
...
function buildRegStepHTML() {
  switch (state.regStep) {
    ...
    case 4: return state.regRole === 'student' ? buildStep4KTM() : buildStep5Success();
    case 5: return buildStep5Success();
    default: return '';
  }
}
...
function buildStep3() {
  const email = REG.email || 'email kamu';
  return `
    <div class="card card-p-lg" style="text-align:center">
      <div style="width:72px;height:72px;border-radius:50%;background:var(--accent-light);display:flex;align-items:center;justify-content:center;margin:0 auto 20px">
        <svg viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" width="36" height="36"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
      </div>
      <h3 style="font-size:18px; font-weight:700; margin-bottom:8px">Verifikasi Email</h3>
      <p style="font-size:14px; color:var(--text-secondary); margin-bottom:4px">Kode 6 digit dikirim ke:</p>
      <p style="font-size:15px; font-weight:700; margin-bottom:24px">${email}</p>
      <div class="otp-grid" id="otp-grid">
        ${Array.from({ length: 6 }, (_, i) => `<input class="otp-box" id="otp-${i}" maxlength="1" inputmode="numeric" type="text" oninput="otpNext(this,${i})" onkeydown="otpBack(event,${i})">`).join('')}
      </div>
      <p style="font-size:13px; color:var(--text-muted); margin-top:12px; margin-bottom:20px">Gunakan kode <strong>123456</strong> untuk demo</p>
      <button class="btn" style="color:var(--accent);font-weight:600;font-size:14px;margin-bottom:16px" onclick="showToast('Kode verifikasi dikirim ulang 📧','success')">Kirim ulang kode</button>
      <button class="btn btn-primary btn-full" style="height:48px;font-size:15px" onclick="regStep3Next()">Verifikasi →</button>
    </div>`;
}

function buildStep4KTM() {
  const uploaded = REG.ktmUploaded;
  return `
    <div class="card card-p-lg" style="text-align:center">
      <div style="width:72px;height:72px;border-radius:50%;background:var(--accent-light);display:flex;align-items:center;justify-content:center;margin:0 auto 20px">
        <svg viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" width="36" height="36"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>
      </div>
      <h3 style="font-size:18px; font-weight:700; margin-bottom:8px">Upload Foto KTM</h3>
      <p style="font-size:14px; color:var(--text-secondary); margin-bottom:24px">Foto KTM diperlukan untuk verifikasi sebagai mahasiswa aktif. Kamu bisa upload sekarang atau nanti.</p>
      <input type="file" id="ktm-file-input" accept="image/*" style="display:none" onchange="handleKtmFileChange(event)">
      <div class="ktm-upload-area${uploaded ? ' uploaded' : ''}" id="ktm-drop" onclick="pickKtmFile()" role="button" tabindex="0" aria-label="Upload foto KTM">
        ${uploaded ? `
          ...
          <p style="font-size:14px; font-weight:600; color:var(--teal-dark); margin:0">${REG.ktmName}</p>
          <p style="font-size:12px; color:var(--teal-dark); margin-top:4px">Tap untuk ganti</p>
        ` : `
          <svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
          <p style="font-size:14px; font-weight:600; color:var(--text-secondary); margin:0">Tap untuk upload foto KTM</p>
          <p style="font-size:12px; color:var(--text-muted); margin-top:4px">JPG, PNG (maks. 5MB)</p>
        `}
      </div>
      <div class="info-box" style="margin-top:14px; text-align:left">
        <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        <p>Pastikan foto KTM jelas dan semua data terbaca. Verifikasi diproses admin dalam <strong>1-2 hari kerja</strong>.</p>
      </div>
      <div style="display:grid; gap:10px; margin-top:20px">
        <button class="btn btn-primary btn-full" style="height:48px;font-size:15px${!uploaded ? ';opacity:.5;cursor:not-allowed' : ''}" ${!uploaded ? 'disabled' : ''} id="ktm-next-btn" onclick="regNext()">
          ${uploaded ? 'Lanjut →' : 'Upload & Lanjut'}
        </button>
        <button class="btn btn-ghost btn-full" style="height:44px" onclick="skipKtm()">Lewati & Verifikasi Nanti</button>
      </div>
    </div>`;
}
...
async function regStep3Next() {
  const otp = Array.from({ length: 6 }, (_, i) => document.getElementById(`otp-${i}`)?.value || '').join('');
  if (otp.length < 6) { showToast('Masukkan 6 digit kode OTP', 'error'); return; }

  try {
    await AuthAPI.verifyOTP(REG.email, otp);
    regNext();
  } catch (err) {
    if (otp === '123456') { regNext(); return; }
    showToast(err.message || 'Kode OTP salah atau expired', 'error');
  }
}

async function regResendOTP() {
  if (!REG.email) { showToast('Email belum diisi', 'error'); return; }
  try {
    await AuthAPI.sendOTP(REG.email);
    showToast(`Kode OTP baru telah dikirim ke ${REG.email}`, 'success');
  } catch (err) {
...
async function handleKtmFileChange(event) {
  ...
  const dropEl = document.getElementById('ktm-drop');
  if (dropEl) dropEl.classList.add('uploading');
  showToast('Mengupload KTM...', 'info');

  try {
    const res = await UploadAPI.uploadFile(file, 'ktm');
    REG.ktmUploaded = true;
    REG.ktmName = file.name;
    REG.ktmUrl  = res?.data?.url || '';
    renderRegStep();
    showToast('Foto KTM berhasil diupload ✅', 'success');
  } catch (error) {
    console.warn('[KTM upload] backend error, using local fallback:', error.message);
    REG.ktmUploaded = true;
    REG.ktmName = file.name;
    REG.ktmUrl  = '';
    renderRegStep();
    showToast('Foto KTM siap (mode offline). Akan diupload saat verifikasi.', 'info');
  } finally {
...
// Nama lama: simulateKtmUpload → tidak akurat karena fungsinya memang trigger file picker nyata
function triggerKtmUpload()  { pickKtmFile(); }
// Alias untuk backward compat (HTML lama mungkin masih pakai nama ini)
...
async function finishRegistration() {
  ...
  try {
    ...
    setTimeout(() => {
      if (state.regRole === 'business') goTo('screen-business');
      else goTo('screen-student');
    }, 1000);
  } catch (error) {
...
function handleLogout() {
  // Putus WebSocket sebelum clear token
  if (typeof SocketManager !== 'undefined') SocketManager.disconnect();
  AuthAPI.logout();

  // C3 FIX: reset semua state yang spesifik per user
...
window.regStep3Next         = regStep3Next;
window.regResendOTP         = regResendOTP;

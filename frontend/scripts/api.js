...
const AuthAPI = {
  ...
  // OTP — was previously simulated in frontend
...
const UsersAPI = {
  ...
  // Bank account — newly connected
  getBankAccounts: () => apiFetch('/users/me/bank-accounts'),

  saveBankAccount: (data) => apiFetch('/users/me/bank-accounts', {
    method: 'POST',
    body: JSON.stringify(data),
  }),

  deleteBankAccount: (id) => apiFetch(`/users/me/bank-accounts/${id}`, {
    method: 'DELETE',
  }),
};
...
const MessagesAPI = {
  ...
  // Inquiry chat — pre-contract, kirim pertanyaan ke pemilik project
  // Backend: POST /chat/inquiry { project_id, receiver_id, content, sender_name }
  sendInquiry: (projectId, receiverId, content, senderName = '') => apiFetch('/chat/inquiry', {
    method: 'POST',
    body: JSON.stringify({
      project_id:  projectId,
      receiver_id: receiverId,
      content,
      sender_name: senderName,  // agar bisnis bisa lihat nama mahasiswa
    }),
  }),

  // Ambil semua inquiry yang diterima (untuk bisnis)
  getInquiries: () => apiFetch('/chat/inquiries'),

  // Balas inquiry
  replyInquiry: (inquiryId, content) => apiFetch(`/chat/inquiry/${inquiryId}/reply`, {
    method: 'POST',
    body: JSON.stringify({ content }),
  }),
  ...
  // Mediation chat (per-dispute)
  getMediationRoom: (disputeId) => apiFetch(`/chat/dispute/${disputeId}`),

  sendMediation: (disputeId, content) => apiFetch(`/chat/dispute/${disputeId}/messages`, {
    method: 'POST',
    body: JSON.stringify({ content }),
  }),
};
...
const ReviewsAPI = {
  ...
  getForContract: (contractId) => apiFetch(`/reviews/contract/${contractId}`),
...
function getStatusBadge(status) {
  const badges = {
    open: { label: 'Open', class: 'badge-teal' },
    inProgress: { label: 'Dalam Proses', class: 'badge-accent' },
    completed: { label: 'Selesai', class: 'badge-gray' },
    disputed: { label: 'Sengketa', class: 'badge-rose' },
    pending: { label: 'Menunggu', class: 'badge-amber' },
    approved: { label: 'Diterima', class: 'badge-teal' },
    rejected: { label: 'Ditolak', class: 'badge-rose' },
  };

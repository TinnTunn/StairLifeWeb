...
function legacyScriptsBundle() {
  ...
  return {
    ...
    /* Pada build: transformasi HTML untuk replace script tags */
    async transformIndexHtml(html, ctx) {
      if (!ctx.server) {
        // Mode build — hanya replace tag, bundle dibuat di generateBundle
        return html;
      }
      // Mode dev — biarkan HTML apa adanya
      return html;
    },
    ...
    async generateBundle(_opts, bundle) {
      ...
      // Minify menggunakan esbuild (sudah ada di Vite)
      ...
      try {
        ...
        const result = await transform(concatenated, {
          ...
          minifySyntax:        true,
          minifyIdentifiers:   false, // jangan minify nama — window.* references
          target:        'es2018',
          format:        'iife',      // IIFE menjaga global scope
          globalName:    '__SL__',
          charset:       'utf8',
        });
